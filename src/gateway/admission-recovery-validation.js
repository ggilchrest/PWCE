import requestSchema from '../../contracts/admission-recovery/request.schema.json' with {type:'json'};
import {admissionRecoveryContracts} from './admission-recovery-contracts.js';
const invalid=()=>Object.assign(new Error('admission recovery request does not match its published contract'),{code:'invalid_request'});
...
export function validateAdmissionRecoveryRequest(payload){
 if(!object(payload)||Buffer.byteLength(JSON.stringify(payload))>admissionRecoveryContracts.maximumRequestBytes||requestSchema.required.some(key=>!Object.hasOwn(payload,key)))throw invalid();
